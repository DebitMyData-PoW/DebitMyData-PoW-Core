...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing debitmydata-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='debitmydata-cli', help='debitmydata-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='DebitMyData address to which the faucet should send')
...
parser.add_argument('debitmydata_cli_args', nargs='*', help='Arguments to pass on to debitmydata-cli (default: -signet)')

args = parser.parse_args()

if args.debitmydata_cli_args == []:
    args.debitmydata_cli_args = ['-signet']


def debitmydata_cli(rpc_command_and_params):
    argv = [args.cmd] + args.debitmydata_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = debitmydata_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = debitmydata_cli(['getnewaddress', 'faucet', 'bech32'])
