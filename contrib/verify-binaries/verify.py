...
"""Script for verifying DebitMyData Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from debitmydata.io and debitmydata.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/debitmydata-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://debitmydata.io"
HOST2 = "https://debitmydata.org"
VERSIONPREFIX = "debitmydata-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the DebitMyData community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"debitmydata_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by debitmydata.io
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the debitmydata release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes debitmydata.org lags behind debitmydata.io.)')
    )
