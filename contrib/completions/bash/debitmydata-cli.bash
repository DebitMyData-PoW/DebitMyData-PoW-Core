# bash programmable completion for debitmydata-cli(1)
# Copyright (c) 2012-2022 The Bitcoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

# call $debitmydata-cli for RPC
_debitmydata_rpc() {
    ...
    $debitmydata_cli "${rpcargs[@]}" "$@"
}

_debitmydata_cli() {
    local cur prev words=() cword
    local debitmydata_cli

    # save and use original argument to invoke debitmydata-cli for -help, help and RPC
    # as debitmydata-cli might not be in $PATH
    debitmydata_cli="$1"
    ...
    case "$cur" in
        ...
        *)
            ...
            if [[ -z "$cur" || "$cur" =~ ^- ]]; then
                helpopts=$($debitmydata_cli -help 2>&1 | awk '$1 ~ /^-/ { sub(/=.*/, "="); print $1 }' )
            fi

            # only parse help if senseful
            if [[ -z "$cur" || "$cur" =~ ^[a-z] ]]; then
                commands=$(_debitmydata_rpc help 2>/dev/null | awk '$1 ~ /^[a-z]/ { print $1; }')
            fi
...
complete -F _debitmydata_cli debitmydata-cli
