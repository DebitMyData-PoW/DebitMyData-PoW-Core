...
be used in DebitMyData Core with the `-asmap` option.
...
to binary before being used with DebitMyData Core.
...
the `debitmydata-cli getnodeaddresses` command.
...
```
debitmydata-cli getnodeaddresses 0 > addrs.json
```
