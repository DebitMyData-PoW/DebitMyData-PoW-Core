...
Provides utilities for managing copyright headers of `The DebitMyData Core
...
gen-debitmydata-conf.sh
===================

Generates a debitmydata.conf file in `share/examples/` by parsing the output from `debitmydatad --help`. This script is run during the
release process to include a debitmydata.conf with the release binaries and can also be run by users to generate a file locally.
...
```bash
BUILDDIR=$PWD/my-build-dir contrib/devtools/gen-debitmydata-conf.sh
```
