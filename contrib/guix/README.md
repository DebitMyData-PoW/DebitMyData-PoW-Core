# Bootstrappable DebitMyData Core Builds

This directory contains the files necessary to perform bootstrappable DebitMyData
Core builds.
...
To build DebitMyData Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      DebitMyData Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/debitmydata-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/debitmydata-core/guix.sigs).
