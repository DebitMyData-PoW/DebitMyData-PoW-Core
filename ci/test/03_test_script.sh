...
(
  ...
  # See https://github.com/DebitMyData-PoW/DebitMyData-PoW-Core/pull/28359#issuecomment-1698694748
...
DEBITMYDATA_CONFIG_ALL="-DBUILD_BENCH=ON -DBUILD_FUZZ_BINARY=ON"
if [ -z "$NO_DEPENDS" ]; then
  DEBITMYDATA_CONFIG_ALL="${DEBITMYDATA_CONFIG_ALL} -DCMAKE_TOOLCHAIN_FILE=$DEPENDS_DIR/$HOST/toolchain.cmake"
fi
if [ -z "$NO_WERROR" ]; then
  DEBITMYDATA_CONFIG_ALL="${DEBITMYDATA_CONFIG_ALL} -DWERROR=ON"
fi
...
bash -c "cmake -S $BASE_ROOT_DIR -B ${BASE_BUILD_DIR} $DEBITMYDATA_CONFIG_ALL $DEBITMYDATA_CONFIG" || (
  cd "${BASE_BUILD_DIR}"
...
if [ "${RUN_TIDY}" = "true" ]; then
  ...
  if ! ( run-clang-tidy-"${TIDY_LLVM_V}" -quiet -load="/tidy-build/libdebitmydata-tidy.so" "${MAKEJOBS}" | tee tmp.tidy-out.txt ); then
