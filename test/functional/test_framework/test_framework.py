...
TMPDIR_PREFIX = "debitmydata_func_test_"
...
class Binaries:
    """Helper class to provide information about debitmydata binaries
    ...
    def node_argv(self, **kwargs):
        "Return argv array that should be used to invoke debitmydatad"
        return self._argv("node", self.paths.debitmydatad, **kwargs)

    def rpc_argv(self):
        "Return argv array that should be used to invoke debitmydata-cli"
        # Add -nonamed because "debitmydata rpc" enables -named by default, but debitmydata-cli doesn't
        return self._argv("rpc", self.paths.debitmydatacli) + ["-nonamed"]

    def tx_argv(self):
        "Return argv array that should be used to invoke debitmydata-tx"
        return self._argv("tx", self.paths.debitmydatatx)

    def util_argv(self):
        "Return argv array that should be used to invoke debitmydata-util"
        return self._argv("util", self.paths.debitmydatautil)

    def wallet_argv(self):
        "Return argv array that should be used to invoke debitmydata-wallet"
        return self._argv("wallet", self.paths.debitmydatawallet)

    def chainstate_argv(self):
        "Return argv array that should be used to invoke debitmydata-chainstate"
        return self._argv("chainstate", self.paths.debitmydatachainstate)

    def _argv(self, command, bin_path, need_ipc=False):
        """Return argv array that should be used to invoke the command. It
        either uses the debitmydata wrapper executable (if DEBITMYDATA_CMD is set or
        need_ipc is True), or the direct binary path (debitmydatad, etc). When
        bin_dir is set (by tests calling binaries from previous releases) it
        always uses the direct path."""
        if self.bin_dir is not None:
            return [os.path.join(self.bin_dir, os.path.basename(bin_path))]
        elif self.paths.debitmydata_cmd is not None or need_ipc:
            # If the current test needs IPC functionality, use the debitmydata
            # wrapper binary and append -m so it calls multiprocess binaries.
            debitmydata_cmd = self.paths.debitmydata_cmd or [self.paths.debitmydata_bin]
            return debitmydata_cmd + (["-m"] if need_ipc else []) + [command]
        else:
            return [bin_path]


class DebitMyDataTestMetaClass(type):
    """Metaclass for DebitMyDataTestFramework.

    Ensures that any attempt to register a subclass of `DebitMyDataTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'DebitMyDataTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("DebitMyDataTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("DebitMyDataTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class DebitMyDataTestFramework(metaclass=DebitMyDataTestMetaClass):
    """Base class for a debitmydata test script.

    Individual debitmydata test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self, test_file):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave debitmydatads and test.* datadir on exit or error")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use debitmydata-cli instead of RPC for all commands")
    ...
    def get_binary_paths(self):
        ...
        binaries = {
            "debitmydata": "DEBITMYDATA_BIN",
            "debitmydatad": "DEBITMYDATAD",
            "debitmydata-cli": "DEBITMYDATACLI",
            "debitmydata-util": "DEBITMYDATAUTIL",
            "debitmydata-tx": "DEBITMYDATATX",
            "debitmydata-chainstate": "DEBITMYDATACHAINSTATE",
            "debitmydata-wallet": "DEBITMYDATAWALLET",
        }
        # Set paths to debitmydata core binaries allowing overrides with environment
        # variables.
        ...
        # DEBITMYDATA_CMD environment variable can be specified to invoke debitmydata
        # wrapper binary instead of other executables.
        paths.debitmydata_cmd = shlex.split(os.getenv("DEBITMYDATA_CMD", "")) or None
        return paths
    ...
    def shutdown(self):
        ...
        rpc_logger = logging.getLogger("DebitMyDataRPC")
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a debitmydatad test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple debitmydatad test nodes"""
        for node in self.nodes:
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the debitmydatad
    ...
    def _start_logging(self):
        ...
        # Format logs the same as debitmydatad's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in debitmydata.conf
    ...
    def skip_if_no_debitmydatad_tracepoints(self):
        """Skip the running test if debitmydatad has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("debitmydatad has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_debitmydatad_zmq(self):
        """Skip the running test if debitmydatad has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("debitmydatad has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if debitmydata-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("debitmydata-wallet has not been compiled")

    def skip_if_no_debitmydata_tx(self):
        """Skip the running test if debitmydata-tx has not been compiled."""
        if not self.is_debitmydata_tx_compiled():
            raise SkipTest("debitmydata-tx has not been compiled")

    def skip_if_no_debitmydata_util(self):
        """Skip the running test if debitmydata-util has not been compiled."""
        if not self.is_debitmydata_util_compiled():
            raise SkipTest("debitmydata-util has not been compiled")

    def skip_if_no_debitmydata_chainstate(self):
        """Skip the running test if debitmydata-chainstate has not been compiled."""
        if not self.is_debitmydata_chainstate_compiled():
            raise SkipTest("debitmydata-chainstate has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if debitmydata-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("debitmydata-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether debitmydata-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether debitmydata-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_debitmydata_tx_compiled(self):
        """Checks whether debitmydata-tx was compiled."""
        return self.config["components"].getboolean("BUILD_DEBITMYDATA_TX")

    def is_debitmydata_util_compiled(self):
        """Checks whether debitmydata-util was compiled."""
        return self.config["components"].getboolean("ENABLE_DEBITMYDATA_UTIL")

    def is_debitmydata_chainstate_compiled(self):
        """Checks whether debitmydata-chainstate was compiled."""
        return self.config["components"].getboolean("ENABLE_DEBITMYDATA_CHAINSTATE")
