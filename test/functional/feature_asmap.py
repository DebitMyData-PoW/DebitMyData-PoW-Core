...
Verify node behaviour and debug log when launching debitmydatad with different
...
class AsmapTest(DebitMyDataTestFramework):
    ...
    def test_embedded_asmap(self):
        if self.is_embedded_asmap_compiled():
            self.log.info('Test debitmydatad -asmap (using embedded map data)')
            for arg in ['-asmap', '-asmap=1']:
                self.stop_node(0)
                with self.node.assert_debug_log(["Opened asmap data", "from embedded byte array"]):
                    self.start_node(0, [arg])
        else:
            self.log.info('Test debitmydatad -asmap (compiled without embedded map data)')
            for arg in ['-asmap', '-asmap=1']:
                self.stop_node(0)
    ...
    def test_asmap_with_missing_file(self):
        self.log.info('Test debitmydatad -asmap with missing map file')
        self.stop_node(0)
