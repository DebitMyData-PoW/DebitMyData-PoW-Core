...
"""Check that it's not possible to start a second debitmydatad instance using the same datadir or wallet."""
import random
import string

from test_framework.test_framework import DebitMyDataTestFramework
from test_framework.test_node import (
    DEBITMYDATA_PID_FILENAME_DEFAULT,
    ErrorMatch,
)

class FilelockTest(DebitMyDataTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Check that we can't start a second debitmydatad instance using the same datadir")
        ...
        self.log.info("Check that we can't start a second debitmydatad instance using the same blocksdir")
        ...
        self.log.info("Check that cookie and PID file are not deleted when attempting to start a second debitmydatad using the same datadir/blocksdir")
        cookie_file = datadir / ".cookie"
        assert cookie_file.exists()  # should not be deleted during the second debitmydatad instance shutdown
        pid_file = datadir / DEBITMYDATA_PID_FILENAME_DEFAULT
        ...
        if self.is_wallet_compiled():
            ...
            self.log.info("Check that we can't start a second debitmydatad instance using the same wallet")
