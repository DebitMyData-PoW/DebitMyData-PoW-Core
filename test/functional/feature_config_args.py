...
class ConfArgsTest(DebitMyDataTestFramework):
    ...
    def test_dir_config(self):
        ...
        conf_path = self.nodes[0].datadir_path / 'debitmydata.conf'
    ...
    def test_negated_config(self):
        ...
        ignored_file_message = [f'Data directory "{self.nodes[0].datadir_path}" contains a "debitmydata.conf" file which is explicitly ignored using -noconf.']
    ...
    def test_config_file_parser(self):
        ...
        # Check that startup fails if conf= is set in debitmydata.conf or in an included conf file
        bad_conf_file_path = self.nodes[0].datadir_path / "debitmydata_bad.conf"
        ...
        with open(self.nodes[0].datadir_path / 'debitmydata.conf', 'a') as conf:
        ...
        main_conf_file_path = self.nodes[0].datadir_path / "debitmydata_main.conf"
    ...
    def test_config_file_log(self):
        ...
        # directory by debitmydatad.
        env, default_datadir = util.get_temp_default_datadir(Path(self.options.tmpdir, "test_config_file_log"))
        default_datadir.mkdir(parents=True)

        # Write a debitmydata.conf file in the default data directory containing a
        # datadir= line pointing at the node datadir.
        node = self.nodes[0]
        conf_text = node.debitmydataconf.read_text()
        conf_path = default_datadir / "debitmydata.conf"
        ...
        # (conf_path, not node.debitmydataconf)
    ...
    def test_privatebroadcast(self):
        ...
        self.nodes[0].assert_start_raises_init_error(
            extra_args=["-privatebroadcast", "-connect=127.0.0.1:18556", "-onion=127.0.0.1:9050"],
            expected_msg=(
    ...
    def test_ignored_conf(self):
        self.log.info('Test error is triggered when the datadir in use contains a debitmydata.conf file that would be ignored '
        ...
        node.assert_start_raises_init_error([f"-conf={temp_conf.name}"], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "debitmydata.conf" file which is ignored, because a '
        ...
        # the same debitmydata.conf that would be loaded anyway does not trigger an
        # error.
        self.start_node(0, [f'-conf={node.datadir_path}/debitmydata.conf'])
        self.stop_node(0)
    ...
    def test_ignored_default_conf(self):
        ...
        self.log.info('Test error is triggered when debitmydata.conf in the default data directory sets another datadir '
                      'and it contains a different debitmydata.conf file that would be ignored')
        ...
        # debitmydata.conf that would be ignored.
        node = self.nodes[0]
        (default_datadir / "debitmydata.conf").write_text(f"datadir={node.datadir_path}\n")
        ...
        node.assert_start_raises_init_error([], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "debitmydata.conf" file which is ignored, because a '
            f'different configuration file "{default_datadir}/debitmydata.conf" from data directory "{default_datadir}" '
    ...
    def test_acceptstalefeeestimates_arg_support(self):
        ...
        conf_file = self.nodes[0].datadir_path / "debitmydata.conf"
    ...
    def run_test(self):
        ...
        conf_file = default_data_dir / "debitmydata.conf"
        ...
        none_existent_conf_file = default_data_dir / "none_existent_debitmydata.conf"
