...
"""Test debitmydata-chainstate tool functionality

Test basic block processing via debitmydata-chainstate tool, including detecting
duplicates and malformed input.

Test that debitmydata-chainstate can load a datadir initialized with an assumeutxo
...
class DebitMyDataChainstateTest(DebitMyDataTestFramework):
    def skip_test_if_missing_module(self):
        self.skip_if_no_debitmydata_chainstate()
    ...
    def basic_test(self):
        ...
        self.log.info(f"Test debitmydata-chainstate {self.get_binaries().chainstate_argv()} with datadir: {datadir}")
    ...
    def assumeutxo_test(self, dump_output_path):
        ...
        datadir = n1.chain_path
        n1.stop_node()
        self.log.info(f"Test debitmydata-chainstate {self.get_binaries().chainstate_argv()} with an assumeutxo datadir: {datadir}")
...
if __name__ == "__main__":
    DebitMyDataChainstateTest(__file__).main()
