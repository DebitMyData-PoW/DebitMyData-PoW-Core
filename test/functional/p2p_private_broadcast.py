...
class P2PPrivateBroadcast(DebitMyDataTestFramework):
    ...
    def run_test(self):
        ...
        for addr in ADDRMAN_ADDRESSES:
            res = tx_originator.addpeeraddress(address=addr, port=0 if addr.endswith(".i2p") else 18556, tried=False)
            if not res["success"]:
        ...
        # Stop the SOCKS5 proxy server to avoid it being upset by the debitmydata
        ...
        self.restart_node(0, extra_args=[
            ...
            # DebitMyData Core to start, hoping/assuming that the location of the Tor proxy
