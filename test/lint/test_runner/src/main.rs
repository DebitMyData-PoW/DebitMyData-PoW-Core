...
fn get_linter_list() -> Vec<&'static Linter> {
    vec![
        ...
        &Linter {
            description: "Check that no symbol from debitmydata-build-config.h is used without the header being included",
...
fn get_pathspecs_exclude_whitespace() -> Vec<String> {
    ...
    list.extend(
        [
            ...
            "contrib/init/debitmydatad.openrc",
...
fn lint_includes_build_config() -> LintResult {
    let config_path = "./cmake/debitmydata-build-config.h.in";
        ...
        git()
            .args([
                ...
                if mode {
                    ...
                    "#include <debitmydata-build-config.h>" // Catch redundant includes with and without the IWYU pragma
                },
                "--",
            ])
    ...
    if missing {
        return Err(format!(
            r#"
One or more files use a symbol declared in the debitmydata-build-config.h header. However, they are not
...
even though debitmydata-build-config.h indicates that a faster feature is available and should be used.
...
#include <debitmydata-build-config.h> // IWYU pragma: keep
    ...
    if redundant {
        return Err(r#"
None of the files use a symbol declared in the debitmydata-build-config.h header. However, they are including
