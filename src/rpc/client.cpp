...
//! Specify whether parameter should be parsed by debitmydata-cli as a JSON value,
 ...
 *
 * JSON parameters need to be listed here to make debitmydata-cli parse command line
 * arguments as JSON, instead of passing them as raw strings. `JSON` and
 * `JSON_OR_STRING` formats both make `debitmydata-cli` attempt to parse the
 ...
 * `debitmydata-cli getblockstats <hash>` without needing to quote the hash string
 * as JSON (`'"<hash>"'`).
 *
 ...
 * to make debitmydata-cli treat them as positional parameters when `-named` is used.
 * This prevents `debitmydata-cli` from splitting strings like "my=wallet" into a named
 ...
 * other string parameters for that method need to be listed as well so debitmydata-cli
 ...
 *
 * For example, the command `debitmydata-cli -named createwallet "my=wallet"`,
