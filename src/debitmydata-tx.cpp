...
static void SetupDebitMyDataTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " debitmydata-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The debitmydata-tx tool is used for creating and modifying debitmydata transactions.\n\n"
                "debitmydata-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded debitmydata transaction, or with \"-create [commands]\" to create a hex-encoded debitmydata transaction.\n"
                "\n"
                "Usage: debitmydata-tx [options] <hex-tx> [commands]\n"
                "or:    debitmydata-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded debitmydata transaction
