...
const char * const DEBITMYDATA_CONF_FILENAME = "debitmydata.conf";
const char * const DEBITMYDATA_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //debitmydata-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", DEBITMYDATA_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\DebitMyData
    //   new: C:\Users\Username\AppData\Local\DebitMyData
    // macOS: ~/Library/Application Support/DebitMyData
    // Unix-like: ~/.debitmydata
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "DebitMyData";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "DebitMyData";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/DebitMyData";
#else
    // Unix-like
    return pathRet / ".debitmydata";
#endif
#endif
}
