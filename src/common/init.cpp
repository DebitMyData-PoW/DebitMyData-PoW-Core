...
std::optional<ConfigError> InitConfig(ArgsManager& args, SettingsAbortFn settings_abort_fn)
{
    try {
        ...
        // debitmydata.conf file just ignores the other file.)
        const fs::path orig_datadir_path{args.GetDataDirBase()};
        const fs::path orig_config_path{AbsPathForConfigVal(args, args.GetPathArg("-conf", DEBITMYDATA_CONF_FILENAME), /*net_specific=*/false)};
        ...
        if (!fs::exists(base_path)) {
            ...
            // in https://github.com/DebitMyData-PoW/DebitMyData-PoW-Core/issues/16220 about ways to
        ...
        const fs::path base_config_path = base_path / DEBITMYDATA_CONF_FILENAME;
        if (fs::exists(base_config_path)) {
            if (orig_config_path.empty()) {
                LogInfo(
                    ...
                    fs::quoted(DEBITMYDATA_CONF_FILENAME));
