...
Options:
  -m, --multiprocess     Run multiprocess binaries debitmydata-node, debitmydata-gui.
  -M, --monolithic       Run monolithic binaries debitmydatad, debitmydata-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show full help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'debitmydata-qt [ARGS]' or 'debitmydata-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'debitmydatad [ARGS]' or 'debitmydata-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'debitmydata-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'debitmydata-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'debitmydata-tx [ARGS]'.
  help           Show full help message.
)";

static constexpr auto HELP_FULL = R"(
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench_debitmydata [ARGS]'.
  chainstate [ARGS] Run debitmydata kernel chainstate util, equivalent to running 'debitmydata-chainstate [ARGS]'.
  test [ARGS]       Run unit tests, equivalent to running 'test_debitmydata [ARGS]'.
  test-gui [ARGS]   Run GUI unit tests, equivalent to running 'test_debitmydata-qt [ARGS]'.
)";
...
int main(int argc, char* argv[])
{
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(UseMultiprocess(cmd) ? "debitmydata-gui" : "debitmydata-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(UseMultiprocess(cmd) ? "debitmydata-node" : "debitmydatad");
        } else if (cmd.command == "rpc") {
            args.emplace_back("debitmydata-cli");
            // Since "debitmydata rpc" is a new interface that doesn't need to be
            ...
            args.emplace_back("debitmydata-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("debitmydata-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench_debitmydata");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("debitmydata-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_debitmydata");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_debitmydata-qt");
        } else if (cmd.command == "util") {
            args.emplace_back("debitmydata-util");
        } else {
...
//! Execute the specified debitmydatad, debitmydata-qt or other command line in `args`
...
//!             be a relative path to a debitmydatad, debitmydata-qt or other executable
...
//!
//!     strace -e trace=execve -s 10000 build/bin/debitmydata ...
...
static void ExecCommand(const std::vector<const char*>& args, std::string_view wrapper_argv0)
{
    ...
    // (https://github.com/DebitMyData-PoW/DebitMyData-PoW-Core/pull/31375#discussion_r1861814807)
