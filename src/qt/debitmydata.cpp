...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaType<DebitMyDataUnit>("DebitMyDataUnit");
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. debitmydata_de.qm (shortcut "de" needs to be defined in debitmydata.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. debitmydata_de_DE.qm (shortcut "de_DE" needs to be defined in debitmydata.qrc)
...
static const char* qt_argv = "debitmydata-qt";

DebitMyDataApplication::DebitMyDataApplication()
...
void DebitMyDataApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the DebitMyDataApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", DebitMyDataGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void DebitMyDataApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool DebitMyDataApplication::createOptionsModel(bool resetSettings)
{
...
void DebitMyDataApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new DebitMyDataGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &DebitMyDataGUI::quitRequested, this, &DebitMyDataApplication::requestShutdown);
...
void DebitMyDataApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void DebitMyDataApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool DebitMyDataApplication::baseInitialize()
{
    return node().baseInitialize();
}

void DebitMyDataApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &DebitMyDataApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &DebitMyDataApplication::handleRunawayException);
    connect(this, &DebitMyDataApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &DebitMyDataApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void DebitMyDataApplication::parameterSetup()
{
...
void DebitMyDataApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void DebitMyDataApplication::requestInitialize()
{
...
void DebitMyDataApplication::requestShutdown()
{
...
void DebitMyDataApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // debitmydata: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &DebitMyDataGUI::handlePaymentRequest);
            connect(window, &DebitMyDataGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void DebitMyDataApplication::handleRunawayException(const QString &message)
{
...
void DebitMyDataApplication::handleNonFatalException(const QString& message)
{
...
WId DebitMyDataApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool DebitMyDataApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", DebitMyDataGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(debitmydata);
    Q_INIT_RESOURCE(debitmydata_locale);
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(DEBITMYDATA_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see debitmydata-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see debitmydata-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse debitmydata.conf, determine network, switch to network specific
    ...
    // debitmydata: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
