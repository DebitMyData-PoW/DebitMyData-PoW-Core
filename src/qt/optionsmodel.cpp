...
/** Call node.updateRwSetting() with DebitMyData 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because DebitMyData 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/DebitMyData-PoW/DebitMyData-PoW-Core/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because debitmydata 22.x will not try to read these.
...
//! Convert enabled/size values to debitmydata -prune setting.
...
//! Get pruning enabled value to show in GUI from debitmydata -prune setting.
...
//! Get pruning size value to show in GUI from debitmydata -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayDebitMyDataUnit")) {
        settings.setValue("DisplayDebitMyDataUnit", QVariant::fromValue(DebitMyDataUnit::DMD));
    }
    QVariant unit = settings.value("DisplayDebitMyDataUnit");
    if (unit.canConvert<DebitMyDataUnit>()) {
        m_display_debitmydata_unit = unit.value<DebitMyDataUnit>();
    } else {
        m_display_debitmydata_unit = DebitMyDataUnit::DMD;
        settings.setValue("DisplayDebitMyDataUnit", QVariant::fromValue(m_display_debitmydata_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if debitmydata.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_debitmydata_unit);
...
{
    if (new_unit.isNull() || new_unit.value<DebitMyDataUnit>() == m_display_debitmydata_unit) return;
    m_display_debitmydata_unit = new_unit.value<DebitMyDataUnit>();
    QSettings settings;
    settings.setValue("DisplayDebitMyDataUnit", QVariant::fromValue(m_display_debitmydata_unit));
    Q_EMIT displayUnitChanged(m_display_debitmydata_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/DebitMyData-PoW/DebitMyData-PoW-Core/pull/8273
    ...
    // (https://github.com/debitmydata-core/gui/issues/567).
