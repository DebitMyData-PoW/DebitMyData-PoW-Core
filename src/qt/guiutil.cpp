...
#include <qt/debitmydataaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a DebitMyData address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new DebitMyDataAddressEntryValidator(parent));
    widget->setCheckValidator(new DebitMyDataAddressCheckValidator(parent));
}
...
bool parseDebitMyDataURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no debitmydata: URI
    if(!uri.isValid() || uri.scheme() != QString("debitmydata"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!DebitMyDataUnits::parse(DebitMyDataUnit::DMD, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseDebitMyDataURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseDebitMyDataURI(uriInstance, out);
}

QString formatDebitMyDataURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("debitmydata:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(DebitMyDataUnits::format(DebitMyDataUnit::DMD, info.amount, false, DebitMyDataUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openDebitMyDataConf()
{
    ...
    /* Open debitmydata.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == ChainType::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "DebitMyData.lnk";
    if (chain == ChainType::TESTNET) // Remove this special case when testnet CBaseChainParams::DataDir() is incremented to "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "DebitMyData (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("DebitMyData (%s).lnk", ChainTypeToString(chain)));
}

bool GetStartOnSystemStartup()
{
    // check for DebitMyData*.lnk
...
fs::path static GetAutostartFilePath()
{
    ChainType chain = gArgs.GetChainType();
    if (chain == ChainType::MAIN)
        return GetAutostartDir() / "debitmydata.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("debitmydata-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a debitmydata.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == ChainType::MAIN)
            optionFile << "Name=DebitMyData\n";
        else
            optionFile << strprintf("Name=DebitMyData (%s)\n", ChainTypeToString(chain));
