This directory contains the source code for the DebitMyData Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
#### debitmydatagui.(h/cpp)

- Represents the main window of the DebitMyData UI.
...
- ClientModel is used by the main application `debitmydatagui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a debitmydata:... link from a browser).
...
* UI elements like DebitMyDataAmountField, which inherit from QWidget.
* `debitmydatastrings.cpp`: automatically generated
* `debitmydataunits.(h/cpp)`: DMD / mDMD / etc. handling
* `callback.h`
...
**Note:** Do not change `local/debitmydata_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `debitmydata-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `debitmydata-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `build/bin`, and select `debitmydata-qt`
9. You're all set! Start developing, building, and debugging the DebitMyData Core GUI
