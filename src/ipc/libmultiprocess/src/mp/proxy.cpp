...
thread_local ThreadContext g_thread_context; // NOLINT(debitmydata-nontrivial-threadlocal)
...
Connection::~Connection()
{
    ...
    // object, which can run arbitrary blocking debitmydata code so they have to run
    ...
    // functions run, so client objects are fully disconnected before debitmydata
    // code in the destructors are run. This way if the debitmydata code tries to
...
void EventLoop::addAsyncCleanup(std::function<void()> fn)
{
    ...
    //
    // In debitmydata core, running these callbacks in the right order is
